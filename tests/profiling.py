...
init_freq = (2*np.pi)/T
plans = pyReSolver.FFTPlans([(init_traj.shape[0] - 1) << 1, init_traj.shape[1]])
...
def do_op():
    _, _, _ = pyReSolver.minimiseResidual(init_traj, init_freq, pyReSolver.systems.lorenz, mean, method = 'L-BFGS-B', plans = plans, iter = 200, psi = psi)
