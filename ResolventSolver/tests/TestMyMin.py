...
class TestMyMin(unittest.TestCase):
    ...
    def test_traj_global_res(self):
        ...
        self.sys2.response(self.mean2, resp_mean_s2)
        lr_t1s1_true = res_funcs.local_residual(self.traj1, self.sys1, self.mean1, H_n_inv_t1s1, self.plan_t1, np.zeros_like(self.traj1), resp_mean_s1)
        lr_t2s1_true = res_funcs.local_residual(self.traj2, self.sys1, self.mean1, H_n_inv_t2s1, self.plan_t2, np.zeros_like(self.traj2), resp_mean_s1)
        lr_t3s2_true = res_funcs.local_residual(self.traj3, self.sys2, self.mean2, H_n_inv_t3s2, self.plan_t3, np.zeros_like(self.traj3), resp_mean_s2)
