...
class TestOptimise(unittest.TestCase):
    ...
    def test_traj_global_res(self):
        res_func_t1s1, _ = init_opt_funcs(self.traj1.shape[0], self.freq1, self.plan_t1, self.sys1, 2, self.mean1)
        res_func_t2s1, _ = init_opt_funcs(self.traj2.shape[0], self.freq2, self.plan_t2, self.sys1, 2, self.mean1)
        res_func_t3s2, _ = init_opt_funcs(self.traj3.shape[0], self.freq3, self.plan_t3, self.sys2, 3, self.mean2)
    ...
    def test_traj_global_res_jac(self):
        _, res_grad_func_t1s1 = init_opt_funcs(self.traj1.shape[0], self.freq1, self.plan_t1, self.sys1, 2, self.mean1)
        _, res_grad_func_t2s1 = init_opt_funcs(self.traj2.shape[0], self.freq2, self.plan_t2, self.sys1, 2, self.mean1)
        _, res_grad_func_t3s2 = init_opt_funcs(self.traj3.shape[0], self.freq3, self.plan_t3, self.sys2, 3, self.mean2)
        ...
        lr_t2s1_true = res_funcs.local_residual(self.traj2, self.sys1, self.mean1, H_n_inv_t2s1, self.plan_t2)
        lr_t3s2_true = res_funcs.local_residual(self.traj3, self.sys2, self.mean2, H_n_inv_t3s2, self.plan_t3)
        gr_traj_t1s1_true = vec2traj(traj2vec(res_funcs.gr_traj_grad(self.traj1, self.sys1, self.freq1, self.mean1, lr_t1s1_true, self.plan_t1)), 2)
        gr_traj_t2s1_true = vec2traj(traj2vec(res_funcs.gr_traj_grad(self.traj2, self.sys1, self.freq2, self.mean1, lr_t2s1_true, self.plan_t2)), 2)
        gr_traj_t3s2_true = vec2traj(traj2vec(res_funcs.gr_traj_grad(self.traj3, self.sys2, self.freq3, self.mean2, lr_t3s2_true, self.plan_t3)), 3)
